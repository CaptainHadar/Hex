...
public PathBoard(char[][] colorsBoard, char playersColor, Position startPos) {
	...
	// נותן לנו את כלל המקומות שבהם הערך הוא כפי שקיבלנו
	...
	// מגדיר לנו את המקומות לפי המחיר במקומות הרוציים ע"פ האלגוריתם
...
//					return;
	...
	// מביא לנו את המחיר הנמוך ביותר של השכנים שלנו
...
//						System.out.println("LowestCost: " + lowestCost);
	...
	// מחזיר את מיקום השכן בעל המחיר הכי נמוך
	...
	// מחזיר תור של כלל השכנים של נקודה מסויימת
...
// מקבלים את השרשרת הקצרה ביותר בשביל לחבר את השביל בשביל ניצחון
	public Queue<Position> getShortestChain() {
		...
		while(i < 150) { // במידה וזה הגיעה כבר לפאוור של 150 זה אומר שהוא בהכרח במבוי סתום.
			// ישנם 3 סוגים של מקרים בלולאה
			// מקרה אחד שהגענו לניצחון 
			// מקרה שני שבו חייל שאנחנו שמנו כבר
			// מקרה שלישי חלק מהשביל שהוא שלא הנחנו בו שחקן
...
// מחזיר לנו את כמות החיילים הדרושים בכדי לחבר את השרשרת
