		...
		if  (!moves.isEmpty()) {
			if (maximizingPlayer) { // בודק אם כרגע אנחנו השחקן שמחפש את המספר המקסימלי 
				...
				while (!moves.isEmpty()){
					...
					char[][] updatedBoard = updateBoardWithMove(board, move, startingPlayer); // משכפל את הלוח ומוסיף לו מהלך במיקום שאנחנו מבקשים בצבע של השחקן המתחיל
				...
				while (!moves.isEmpty()) {
					Position move = moves.remove();
					char[][] updatedBoard = updateBoardWithMove(board, move , getOtherPlayer(startingPlayer)); // משכפל את הלוח ומוסיף לו מהלך במיקום שאנחנו מבקשים בצבע של השחקן השני
